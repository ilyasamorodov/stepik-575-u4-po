...
class ProductPage(BasePage):
    ...
    def has_disappeared_success_message(self):
        assert self.is_disappeared(*ProductPageLocators.SUCCESS_MESSAGE_TEXT), \
            "Unexpected success message found!"
    ...
    def product_price_equals_basket_total(self):
        assert self.get_product_price() in self.get_basket_total_price(), \
            "Product and basket prices are not equal!"
    ...
    def should_be_product_promo_link(self):
        assert self.is_current_url_valid(ProductPageLocators.PRODUCT_PROMO_LINK), \
            "Not a product promo page!"

    def success_message_contains_product_title(self):
        assert self.get_product_title() in self.get_success_message_text(), \
            "No product title in success add to basket message"
